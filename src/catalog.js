...
export const COMMANDS = [
  ...
  { name: 'friday:memory', description: 'View and manage project memory', kind: 'memory-discovery' },
  { name: 'friday:find-skill', description: 'Search and discover available skills', kind: 'memory-discovery' },
  { name: 'friday:api', description: 'API design patterns', kind: 'knowledge-guide' },
  { name: 'friday:frontend', description: 'Frontend development patterns', kind: 'knowledge-guide' },
  { name: 'friday:backend', description: 'Backend development patterns', kind: 'knowledge-guide' },
  { name: 'friday:deploy', description: 'Deployment workflows and CI/CD patterns', kind: 'knowledge-guide' },
...
export const COMMAND_SECTIONS = COMMAND_KIND_ORDER.map((kind) => ({
  ...
  commands: COMMANDS.filter((command) => command.kind === kind).map(({ name, description }) => ({ name, description })),
}));
...
export const MEMORY_TEMPLATES = ['INDEX.md', '_example-decision.md', '_example-preference.md', '_example-pattern.md'];
